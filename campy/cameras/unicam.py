...
def GrabFrames(cam_params, device, writeQueue, dispQueue, stopQueue):
    ...
    #if not grabdata:
    #    raise NameError('grabdata did not work')
    #print(grabdata)
    ...
    grabbing = cam.StartGrabbing(camera) # BeginAcqisition
    print(cam_params["cameraName"], "ready to trigger.")
    #print(camera.IsStreaming())

    frameNumber = 0
    while(grabbing & camera.IsStreaming()):
        if stopQueue or frameNumber >= grabdata["numImagesToGrab"]:
            cam.CloseCamera(cam_params, camera, grabdata) #EndAcquisition and DeInit
        ...
        try:
            ...
            #print("GrabFrame")
            grabResult = cam.GrabFrame(camera, frameNumber) # camera.GetNextImage()

            # Append numpy array to writeQueue for writer to append to file
            #print("GetImage")
            img = cam.GetImageArray(grabResult, cam_params) # grabResult.GetNDArray()
            #print(type(img))
            ...
            cam.ReleaseFrame(grabResult) # grabResult.Release()
